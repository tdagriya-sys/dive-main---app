...
// Drives a portaled overlay's enter/exit animation via manual state instead
// of relying on framer-motion's own initial/animate/exit auto-trigger — that
// auto-trigger turned out to be genuinely UNRELIABLE (not just "needs more
// time") for components rendered through createPortal(..., document.body)
// in this app: verified directly via computed style across every full-screen
// sheet that uses one (ShareCard, GetStartedPopup, SimulateSheet,
...
// them needs this. The precise trigger inside framer-motion/AnimatePresence
// for a portaled target was never conclusively pinned down (a genuine
// upstream/environment quirk, not app code); this sidesteps it entirely with
// a plain, always-reliable prop-change animation instead — the same code
// path any already-mounted `motion` component's `animate` update already
// uses correctly everywhere else in this app (e.g. Widgets.jsx's
// AnimatedNumber).
//
// Usage:
