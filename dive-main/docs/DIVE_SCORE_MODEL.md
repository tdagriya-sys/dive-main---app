...
**"Close the apparent-vs-real gap" card — same X-Ray-class bug (§7.9), fixed 2026-09-01:** this `WhatIfSheet` card originally showed Real Diversification % (`before`) and what it becomes once cross-segment overlap hits 0% (`after`) — mathematically exactly `apparentDiversificationPct`, since `realDiversificationPct = apparent × (1 − overlapShare)` and overlapShare = 0 makes real == apparent by construction (§6.1). It computed both from this screen's own local `apparentDiversification(h)`/`realDiversification(h)` (diveEngine.js), which only detects overlap via a crude match on each **holding's own name** (`crossSegmentOverlaps`) — not the backend's real lookthrough/connectedness engine (§7's tiered model: MF top-holdings, keyword affinity, industry affinity, same-sector). A user genuinely saw Home's canonical tiles read Apparent 70% / Real 69% while this card showed 70/70 (no gap at all) for the identical portfolio — the backend caught a real cross-class connection this screen's local heuristic couldn't see. Fixed the same way §7.9 fixed X-Ray's donut: anchor on `scoreBreakdown.apparentDiversificationPct`/`realDiversificationPct` when available (`hasHoldings: true`), falling back to the local calc only when no canonical score exists yet.

**Same day, follow-up: unified to a Divve Score delta, matching the other two cards.** The user pointed out this card repeated Home's own dashboard numbers verbatim rather than showing "how would my score change" like `cap-issuer`/`fill-categories` do — so it now shows a Divve Score `before`/`after` in the same "Now"/"If fixed" format, with the apparent/real % folded into the card's description text instead of the headline. Unlike those two cards' estimates (which re-run the frontend's `diveScore()` on a hypothetical holdings list, since a full backend recompute isn't available client-side), this delta is **exact, not approximate**: closing the gap only changes whether cross-class overlap exists — it doesn't touch apparent diversification, name diversification, or within-class spread, the other 3 terms of §6.4's concentration formula. So the concentration sub-score's exact swing is just real diversification's own documented weight (0.15) applied to a gap already known exactly from real backend numbers:
```
gapConcentrationDelta = (canonicalApp − canonicalReal) × 0.15   // §6.4's realDiversificationPct weight
gapScoreDelta         = gapConcentrationDelta × scoreBreakdown.weights.concentration   // 0.17, read live
after                 = clamp(canonicalScore + gapScoreDelta, 0, 100)
```
See `buildRealScenarios()`'s `canonicalApp`/`canonicalReal`/`gapScoreDelta`.
...
| 2026-09-01 | `WhatIfSheet`'s "Close the apparent-vs-real gap" card changed from showing Real/Apparent Diversification % as its headline back to the same Divve Score "Now"/"If fixed" format as the other two cards, with the diversification % moved into the description text. The score delta is computed EXACTLY (not a fabricated/estimated figure): closing the gap only changes cross-class overlap, leaving apparent/name/within-class diversification untouched, so the concentration sub-score's swing is exactly `(canonicalApp − canonicalReal) × 0.15` (real diversification's documented weight, §6.4), scaled by `weights.concentration` into the composite — see §11's new note | User pointed out the card repeated Home's own dashboard numbers rather than showing "how would my score be impacted," like the other two What If cards do | `Suggestions.jsx`, `Suggestions.test.jsx`, `docs/DIVE_SCORE_MODEL.md` |
...
export const IDEAL_RANGES = {
  Conservative: { Equity: [20, 30], "Mutual Funds": [15, 25], Bonds: [20, 30], "Gold/Silver": [8, 12], "REIT/InvIT": [5, 10], "FD/RD": [10, 20], PF: [10, 18], ETF: [3, 8], Insurance: [5, 10], Crypto: [0, 2] },
  Balanced:     { Equity: [25, 35], "Mutual Funds": [20, 30], Bonds: [15, 25], "Gold/Silver": [8, 12], "REIT/InvIT": [8, 12], "FD/RD": [8, 15],  PF: [8, 14],  ETF: [5, 10], Insurance: [3, 7],  Crypto: [0, 5] },
  Aggressive:   { Equity: [35, 50], "Mutual Funds": [20, 30], Bonds: [5, 15],  "Gold/Silver": [5, 10], "REIT/InvIT": [8, 15], "FD/RD": [3, 8],   PF: [3, 6],   ETF: [5, 12], Insurance: [2, 5],  Crypto: [2, 8] },
};
```
...
- `prefs.returnExpectation` (Modest/Moderate/High) nudges growth-tier categories (`RETURN_TIER`: Equity/ETF/Crypto = high, Gold/REIT/MutualFunds = medium, FD/RD/Bonds/Insurance = low) up or down the order via `RETURN_BIAS`.
