...
2. You'll see a list of lines like `SETTING_NAME=`. Using your keyboard's arrow keys to move around, edit the following lines (leave any line not mentioned here exactly as it already is):
   ...
   - `EMAIL_API_KEY=`, `OPENAI_API_KEY=`, `ANTHROPIC_API_KEY=`, `CRYPTO_PRICE_API_KEY=`, and the `FINVU_*` lines are all optional extras (real sign-up codes by email, Bot Scan / file upload extraction, live crypto prices, real account-linking) — see `docs/GETTING_API_KEYS.md` for how to get each one, whenever you're ready. You can leave them as placeholders for now and the app will still run, just in a limited "demo" way for those specific features. `OPENAI_API_KEY` (GPT-5.6 Terra) and `ANTHROPIC_API_KEY` (Claude Sonnet 5) work together as primary + automatic fallback for Bot Scan/file upload — setting just one still works, but setting both is the most reliable option.
...
> 🔵 **As of 2026-09-03, if your live site predates this date:** it's running from before the paid (Rs. 99) resilience-score PDF download went live (Razorpay integration — see `docs/RAZORPAY_SETUP_GUIDE.md`), the FD/RD category rename, and the new Refund Policy page. `git pull` + rebuild + restart picks up all the code, but two extra one-time steps are needed:
> 1. **`cd backend && npm install`** before `npm run build` — this update adds a new backend dependency (`razorpay`), which the usual `npm run build` alone won't fetch.
> 2. **Add the Razorpay env vars to the server's `backend/.env`** (`nano .env` — this file is gitignored, so `git pull` never touches it): `RAZORPAY_KEY_ID=`, `RAZORPAY_KEY_SECRET=`, `RAZORPAY_WEBHOOK_SECRET=`, `REPORT_PRICE_PAISE=9900`. See `docs/RAZORPAY_SETUP_GUIDE.md` for where the key values come from. Leaving these as placeholders isn't broken — the download just quietly runs in a clearly-labeled Dev Mode (simulated payment, no real charge) instead of actually charging anyone — but real values mean **real money gets charged the moment someone clicks download**, so make sure Razorpay's website/app approval for your domain is done first.
>
> No Nginx changes, no database migration, and no one gets logged out for this update — the new `/api/payments/...` routes ride the existing `/api/` proxy block, and the new `Payment` collection/`User.portfolioVersion` field are created automatically on first write.
