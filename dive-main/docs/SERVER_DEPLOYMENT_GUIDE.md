...
16. Write this IP address down in your text file — it looks like four numbers separated by dots, e.g. `52.14.201.88`. You'll need it constantly for the rest of this guide. This address is now permanently yours as long as the server exists.
...
2. First, protect your key file so only you can read it. Type this, replacing the path with wherever you saved the `.pem` file (if it's in your Downloads folder, that's fine for this one command):
   ```
   icacls "C:\path\to\divve-key.pem" /inheritance:r
   icacls "C:\path\to\divve-key.pem" /grant:r "%username%:R"
   ```
...
3. Now connect to your server. Type this, replacing the path and the IP address with your own from Part 2:
   ```
   ssh -i "C:\path\to\divve-key.pem" ubuntu@52.14.201.88
   ```
...
7. Type this, replacing the URL with the one you just copied:
   ```
   git clone https://github.com/yourname/dive-main.git divve
   cd divve
   ```
   Your code is now on the server, inside a folder called `divve`.
...
1. Still inside `~/divve/backend` on the server, install everything the app needs:
   ```
   npm install
   ```
...
2. This file will be empty — type (or paste) exactly this, replacing `yourdomain.com` with your real domain in both places it appears:
   ...
   server {
       ...
       location / {
           root /home/ubuntu/divve/frontend/build;
           try_files $uri /index.html;
       }
   }
   ```
3. Save and exit: **Ctrl+O**, **Enter**, **Ctrl+X**.
4. Turn this configuration on:
   ```
...
6. Restart Nginx to apply everything:
   ```
...
4. You'll see a list of "records." Look for one with Type **"A"** and Name **"@"**.
   - If it exists, click the pencil/edit icon next to it, and change its **"Value"** to your Elastic IP address from Part 2 (e.g. `52.14.201.88`). Save.
...
2. On the server: `cd ~/divve`, then `git pull`.
